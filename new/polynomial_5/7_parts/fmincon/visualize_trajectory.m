tc = t_step:t_step:t1+t2+t3;
tf = (t1+t2+t3+t_step):t_step:(t1+t2+t3+t4);
tr = (t1+t2+t2+t3+t4+t_step):t_step:(t1+t2+t3+t4+t5+t6+t7);

%%
z1 = [traj1_z(1,:) traj2_z(1,:) traj3_z(1,:)];
y1 = y(1,1:length(z1));
%%
z2 = traj4_z(1,:);
y2 = y(1,length(z1)+1:length(z1)+length(z2));
%%
z3 = [traj5_z(1,:) traj6_z(1,:) traj7_z(1,:)];
